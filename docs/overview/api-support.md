...
| --- | --- | --- | --- |
| Loader | `LoadLibraryA/W`, `LoadLibraryExA/W`, `FreeLibrary`, `GetProcAddress`, `GetModuleHandleA/W`, `GetModuleFileNameA/W` | Experimental | Supports tested name/ordinal export lookup, import-by-ordinal, load counts, pinning, and basic acyclic import dependency ownership. Complex dependency graphs still need workload validation. See [Loader](../runtime/loader.md). |
