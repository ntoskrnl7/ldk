...
| Threadpool | `QueueUserWorkItem`, `CreateThreadpoolWork`, `SubmitThreadpoolWork`, `WaitForThreadpoolWorkCallbacks`, `CloseThreadpoolWork`, `CreateThreadpoolCleanupGroup`, `CloseThreadpoolCleanupGroup`, `CloseThreadpoolCleanupGroupMembers`, `FreeLibraryWhenCallbackReturns` | Partial | Implemented over kernel work items for default-pool work callbacks. Tested callback environments cover priority, long/persistent hints, race-with-DLL, cleanup groups, and cleanup cancel callbacks; custom pools, timers, waits, I/O objects, activation contexts, and finalization callbacks are not implemented. |
